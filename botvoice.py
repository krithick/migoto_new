...
#     BotVoiceBase, BotVoiceCreate, BotVoiceResponse, BotVoiceDB,
#      UserRole
# )
...
#     if bot_voice_dict.get("_id") is None:
#         bot_voice_dict.pop("_id", None)
#     else:
#         bot_voice_dict["_id"] = UUID(str(bot_voice_dict["_id"]))
    
#     result = await db.bot_voices.insert_one(bot_voice_dict)
#     created_bot_voice = await db.bot_voices.find_one({"_id": result.inserted_id})
...
#         if bot_voice.get("created_by") != updated_by:
#             raise HTTPException(
...
#     updated_bot_voice = await db.bot_voices.find_one({"_id": bot_voice_id})
...
#         if bot_voice.get("created_by") != deleted_by:
#             raise HTTPException(
#                 status_code=status.HTTP_403_FORBIDDEN,
...
#     # Check if this bot voice is being used in any avatar interactions
#     avatar_interactions_using_voice = await db.avatar_interactions.find(
#         {"bot_voices": bot_voice_id}
#     ).to_list(length=1)
...
#     # Delete the bot voice
#     result = await db.bot_voices.delete_one({"_id": bot_voice_id})
