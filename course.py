...
# # from main import db   # Import your database instance directly
...
#             cursor = db.courses.find({"_id": {"$in": assigned_course_ids}}).skip(skip).limit(limit)
...
#     if course_dict.get("_id") is None:
#         course_dict.pop("_id", None)
#     else:
#         course_dict["_id"] = UUID(str(course_dict["_id"]))
    
#     # Convert module UUIDs to proper format
#     if "modules" in course_dict and course_dict["modules"]:
#         course_dict["modules"] = [UUID(str(module_id)) if not isinstance(module_id, UUID) else module_id 
#                                 for module_id in course_dict["modules"]]
...
#         if course.get("created_by") != updated_by:
#             raise HTTPException(
...
#     # Convert module UUIDs if present
#     if "modules" in course_updates and course_updates["modules"]:
#         course_updates["modules"] = [UUID(str(module_id)) if not isinstance(module_id, UUID) else module_id 
#                                    for module_id in course_updates["modules"]]
...
#     updated_course = await db.courses.find_one({"_id": course_id})
#     if updated_course:
...
#         if course.get("created_by") != deleted_by:
#             raise HTTPException(
#                 status_code=status.HTTP_403_FORBIDDEN,
