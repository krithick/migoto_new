...
#     AvatarInteractionBase, AvatarInteractionCreate, AvatarInteractionResponse, AvatarInteractionDB,
#      UserRole
# )
...
#         bot_voice = await db.bot_voices.find_one({"_id": bot_voice_id})
#         if not bot_voice:
#             raise HTTPException(
#                 status_code=status.HTTP_400_BAD_REQUEST,
...
#     avatar_interactions = []
#     cursor = db.avatar_interactions.find({"personas": persona_id}).skip(skip).limit(limit)
#     async for document in cursor:
#         avatar_interactions.append(AvatarInteractionDB(**document))
...
#     avatar_interactions = []
#     cursor = db.avatar_interactions.find({"avatars": avatar_id}).skip(skip).limit(limit)
#     async for document in cursor:
#         avatar_interactions.append(AvatarInteractionDB(**document))
...
#     avatar_interactions = []
#     cursor = db.avatar_interactions.find({"environments": environment_id}).skip(skip).limit(limit)
#     async for document in cursor:
#         avatar_interactions.append(AvatarInteractionDB(**document))
...
#     if avatar_interaction_dict.get("_id") is None:
#         avatar_interaction_dict.pop("_id", None)
#     else:
#         avatar_interaction_dict["_id"] = UUID(str(avatar_interaction_dict["_id"]))
    
#     # Convert all UUIDs in the lists
...
#             avatar_interaction_dict[field] = [
#                 UUID(str(item_id)) if not isinstance(item_id, UUID) else item_id 
#                 for item_id in avatar_interaction_dict[field]
#             ]
    
#     result = await db.avatar_interactions.insert_one(avatar_interaction_dict)
#     created_avatar_interaction = await db.avatar_interactions.find_one({"_id": result.inserted_id})
...
#     """
#     # Get the avatar interaction to update
#     avatar_interaction = await db.avatar_interactions.find_one({"_id": avatar_interaction_id})
...
#         if avatar_interaction.get("created_by") != updated_by:
#             raise HTTPException(
...
#             avatar_interaction_updates[field] = [
#                 UUID(str(item_id)) if not isinstance(item_id, UUID) else item_id 
#                 for item_id in avatar_interaction_updates[field]
#             ]
    
#     # Update in database
#     await db.avatar_interactions.update_one(
#         {"_id": avatar_interaction_id},
#         {"$set": avatar_interaction_updates}
#     )
    
#     updated_avatar_interaction = await db.avatar_interactions.find_one({"_id": avatar_interaction_id})
...
#     """
#     # Get the avatar interaction to delete
#     avatar_interaction = await db.avatar_interactions.find_one({"_id": avatar_interaction_id})
#     if not avatar_interaction:
#         return False
...
#         if avatar_interaction.get("created_by") != deleted_by:
#             raise HTTPException(
#                 status_code=status.HTTP_403_FORBIDDEN,
...
#     # Check if this avatar interaction is being used in any scenarios
...
#     # Delete the avatar interaction
#     result = await db.avatar_interactions.delete_one({"_id": avatar_interaction_id})
