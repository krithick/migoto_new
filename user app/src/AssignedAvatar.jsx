...
const AssignedAvatar = () => {
    ...
    if (selected != null) {
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          // name: selected.name,
          // age: selected.age,
          // persona_type: selected.persona_id[0].persona_type,
          // persona_details: selected.persona_id[0].persona_details,
  ...
  const fetchAssignedAvatarDetails = async () => {
    ...
    try {
      ...
      instance
        .get(
          `/avatar-interactions/${avatarIntraction?.avatar_interaction}?expand=avatars`,
          {
            headers,
          }
        )
        .then((response) => {
          setAvatarDetails(response.data.avatars);
          if (selected == null) setSelected(response.data.avatars[0]);
        })
        .catch((error) => {
          console.error("Error fetching assigned avatar:", error);
        });
    } catch (error) {
      console.error("Error fetching assigned avatar:", error);
    }
              ...
              <div className="h-full w-[80rem]">
                  ...
                  <div className="flex flex-auto justify-start">
                    <div>
                      ...
                      <div className="text-[12px] xl:text-[10px] 2xl:text-[12px] text-[#131F49]">
                        Assigned Avatars: {avatarDetails?.length}
                      </div>
                    </div>
                  </div>
                ...
                <div className="h-[40.8rem] xl:h-[24rem] 2xl:h-[39rem] w-full pl-[3.5rem] xl:pl-[2rem] 2xl:pl-[3.5rem] pr-[22px] xl:pr-[14px] 2xl:pr-[22px] py-[15px] xl:py-[10px] 2xl:py-[15px]">
                  <div className="h-full w-full flex flex-wrap gap-x-[5rem] xl:gap-x-[2.5rem] 2xl:gap-x-[4.4rem] space-y-[36px] overflow-y-auto [&::-webkit-scrollbar]:w-[6px] [&::-webkit-scrollbar-track]:bg-[#d3e4ebef] [&::-webkit-scrollbar-thumb]:bg-[#3E4580] [&::-webkit-scrollbar-track]:rounded-[22px] [&::-webkit-scrollbar-thumb]:rounded-[22px]">
                    {avatarDetails && avatarDetails?.length == 0 && (
                      <>
                        <div className="h-full w-full flex px-3 py-3 gap-x-[5rem] xl:gap-x-[2rem] 2xl:gap-x-[3rem] bg-[#FFFFFF8F] rounded-[10px] justify-center items-center">
                          No available avatars, please contact to admin
                        </div>
                      </>
                    )}

                    {avatarDetails &&
                      avatarDetails.map((value, index) => (
                        <>
                          {filter != "" ? (
                            <>
                              {value.persona_id[0].name
                                .toLowerCase()
                                .includes(filter) ? (
                                <div
                                  key={index}
                                    ...
                                    <div className="h-[45%] w-full">
                                      <div className="h-[4.4rem] xl:h-[6.3rem] 2xl:h-[7.5rem] w-full space-y-0.5">
                                        ...
                                        <div className="text-[12px] xl:text-[10px] 2xl:text-[12px] text-[#000000ce] font-normal">
                                          Age: {value.persona_id[0]?.age}
                                        </div>
                                        <div className="text-[12px] xl:text-[10px] 2xl:text-[12px] text-[#000000ce] font-normal">
                                          Profession:{" "}
                                          {value.persona_id[0]?.persona_type}
                                        </div>
                                        <div className="text-[12px] xl:text-[10px] 2xl:text-[12px] text-[#0000008a] font-normal line-clamp-3">
                                          {value.persona_id[0]?.persona_details}
                                        </div>
                                      </div>
