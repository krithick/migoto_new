...
# Fix for create_user
# async def create_user(db: Any, user: UserCreate) -> UserDB:
#     """Create a new user"""
...
#     if admin_dict.get("_id") is None:
#         admin_dict.pop("_id", None)
#     else:
#         admin_dict["_id"] = UUID(str(admin_dict["_id"]))
    
#     # Convert managed_users UUIDs
#     if "managed_users" in admin_dict and admin_dict["managed_users"]:
#         admin_dict["managed_users"] = [UUID(str(user_id)) if not isinstance(user_id, UUID) else user_id 
#                                       for user_id in admin_dict["managed_users"]]
...
#     users = []
#     if managed_user_ids:
#         cursor = db.users.find({"_id": {"$in": managed_user_ids}})
#         async for document in cursor:
#             users.append(UserDB(**document))
...
async def upsert_assignment(db, user_id: UUID, course_assignments: List[CourseAssignmentUpdate], operation: str):
    existing = await db.user_course_assignments.find_one({"user_id": str(user_id)})

    if operation == "add":
        if existing:
            updated_assignments = existing.get("course_assignments", [])

            for incoming_course in course_assignments:
                found_course = next((c for c in updated_assignments if c["course_id"] == str(incoming_course.course_id)), None)
                if not found_course:
                    updated_assignments.append({
                        "course_id": str(incoming_course.course_id),
                        "modules_assigned": [
                            {
                                ...
                                "scenarios_assigned": [
                                    {
                                        ...
                                        "learn_mode": s.learn_mode,
                                        "try_mode": s.try_mode,
                                        "assess_mode": s.assess_mode,
                                    } for s in mod.scenarios_assigned
                                ]
                            } for mod in incoming_course.modules_assigned
                        ]
                    })
                else:
                    for mod in incoming_course.modules_assigned:
                        found_module = next((m for m in found_course["modules_assigned"] if m["module_id"] == str(mod.module_id)), None)
                        if not found_module:
                            found_course["modules_assigned"].append({
                                "module_id": str(mod.module_id),
                                "scenarios_assigned": [
                                    {
                                        "scenario_id": str(s.scenario_id),
                            ...
                            for s in mod.scenarios_assigned:
                                found_scenario = next((sc for sc in found_module["scenarios_assigned"] if sc["scenario_id"] == str(s.scenario_id)), None)
                                if not found_scenario:
                                    found_module["scenarios_assigned"].append({
                                        "scenario_id": str(s.scenario_id),
                                        "learn_mode": s.learn_mode,
                                        "try_mode": s.try_mode,
                                        "assess_mode": s.assess_mode,
                                    ...
                                    if s.learn_mode is not None:
                                        found_scenario["learn_mode"] = s.learn_mode
                                    if s.try_mode is not None:
                                        found_scenario["try_mode"] = s.try_mode
                                    if s.assess_mode is not None:
                                        found_scenario["assess_mode"] = s.assess_mode

            await db.user_course_assignments.update_one(
                {"user_id": str(user_id)},
                {"$set": {"course_assignments": updated_assignments}}
            )

        else:
            await db.user_course_assignments.insert_one({
                "user_id": str(user_id),
                "course_assignments": [
                    {
                        ...
                        "modules_assigned": [
                            ...
                            } for mod in course.modules_assigned
                        ]
                    } for course in course_assignments
                ]
            })

    elif operation == "remove" and existing:
        updated_assignments = existing.get("course_assignments", [])

        for incoming_course in course_assignments:
            course_id = str(incoming_course.course_id)
            found_course = next((c for c in updated_assignments if c["course_id"] == course_id), None)
            if not found_course:
                continue

            for mod in incoming_course.modules_assigned:
                mod_id = str(mod.module_id)
                found_module = next((m for m in found_course["modules_assigned"] if m["module_id"] == mod_id), None)
                if not found_module:
                    continue

                for s in mod.scenarios_assigned:
                    scenario_id = str(s.scenario_id)
                    found_module["scenarios_assigned"] = [sc for sc in found_module["scenarios_assigned"] if sc["scenario_id"] != scenario_id]

                # Remove module if empty
                if not found_module["scenarios_assigned"]:
                    found_course["modules_assigned"] = [m for m in found_course["modules_assigned"] if m["module_id"] != mod_id]

            # Remove course if no modules left
            if not found_course["modules_assigned"]:
                updated_assignments = [c for c in updated_assignments if c["course_id"] != course_id]
...
async def assign_courses_to_user_endpoint(
    assignmentList: CourseAssignmentUpdate,
    db: Any = Depends(get_database),
    admin_user: UserDB = Depends(get_admin_user)
):
    """
    ...
    # If admin (not superadmin), check if they manage this user
    if admin_user.role == UserRole.ADMIN:
        managed_users = [str(uid) for uid in admin_user.dict().get("managed_users", [])]
        unauthorized_users = [str(u.user_id) for u in assignmentList.assignments if str(u.user_id) not in managed_users]
        if unauthorized_users:
            raise HTTPException(status_code=403, detail=f"Not authorized to assign courses to users: {unauthorized_users}")

    for assignment in assignmentList.assignments:
        await upsert_assignment(db, assignment.user_id, assignment.course_ids, assignmentList.operation)
    return {"success": True}
