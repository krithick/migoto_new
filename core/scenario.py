...
@router.post("/modules/{module_id}/scenarios", response_model=ScenarioResponse)
async def create_scenario_endpoint(
    ...
    learn_mode: str = Form(...),
    try_mode: str = Form(...),
    assess_mode: str = Form(...),
    db: Any = Depends(get_database),
    admin_user: UserDB = Depends(get_admin_user)
):
    # Save uploaded file
    file_ext = os.path.splitext(thumbnail_file.filename)[1]
    file_name = f"{uuid4()}{file_ext}"
    file_path = f"static/thumbnails/{file_name}"
    os.makedirs(os.path.dirname(file_path), exist_ok=True)
    with open(file_path, "wb") as f:
        f.write(await thumbnail_file.read())
    
    # Construct thumbnail URL (could be full URL if served over HTTP)
    thumbnail_url = f"/{file_path}"

    # Parse JSON strings into Pydantic models
    learn_mode_obj = LearnModeCreate(**json.loads(learn_mode))
    try_mode_obj = TryModeCreate(**json.loads(try_mode))
    assess_mode_obj = AssessModeCreate(**json.loads(assess_mode))

    # Construct ScenarioCreate manually
    scenario = ScenarioCreate(
        title=title,
        description=description,
        thumbnail_url=thumbnail_url,
        learn_mode=learn_mode_obj,
        try_mode=try_mode_obj,
        assess_mode=assess_mode_obj,
    )
